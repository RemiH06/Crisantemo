...
Esto todavía es una promesa de diseño, no una garantía verificada en código: cuando se construya el backend (Fase 2), debe existir una forma concreta de comprobarlo (por ejemplo, un test que confirme que no se escribe a disco ni se registra el audio en logs), no solo dejarlo como texto en el README.
...
**Limitación importante y sin resolver todavía:** Common Voice es voz leída de forma natural por voluntarios, nadie ahí está practicando deliberadamente cambiar su registro de voz. El caso de uso real de Crisantemo (alguien practicando separar tono de resonancia a propósito) casi no tiene representación en ese tipo de dataset, sin importar cuánto volumen se le meta; se confirmó esto probando con voz actuada real, donde el modelo se sigue dejando engañar por el tono en casos concretos (detalle técnico completo en `docs/TRAINING_REPRODUCTION.md` secciones 4.3-4.4 y `docs/ARCHITECTURE.md`). Esto es relevante aquí, en el documento de ética, y no solo en los documentos técnicos, porque es exactamente el tipo de usuario al que Crisantemo dice servir (alguien practicando su voz) el que más puede toparse con esta limitación. Hasta que se resuelva, conviene comunicar esto con claridad en la interfaz cuando corresponda, no solo dejarlo documentado internamente.
