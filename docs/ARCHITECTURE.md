...
**Tercera actualización, sesión posterior:** se subió el peso adversarial de 5x a 10x (barrido completo 5-20 en `docs/TRAINING_REPRODUCTION.md` sección 4.10), con mejora clara y monótona en la prueba automática (accuracy en adversariales sintéticos 85.5%→91.7%, costo de 81.7%→79.5% en accuracy general). Modelo reexportado a `models/crisantemo_v1.joblib` con peso 10. Pero se encontró un riesgo real al verificar con la prueba de estrés manual (sección 5): Praat mide mal F1 en voz sintética grave (F0=100Hz) con formantes masculinos, y subir el peso adversarial hace que el modelo dependa más de `f2_f1_diff_hz`, amplificando ese ruido de medición hacia el lado femenino en un caso que debería ser masculino sin ambigüedad. **Pendiente de confirmar con voz real** si este riesgo también afecta grabaciones reales de voz grave (no solo la síntesis de prueba) antes de dar el modelo con peso 10 por bueno. Detalle completo en la sección 4.10.
