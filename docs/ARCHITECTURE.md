...
El modelo actual (`models/crisantemo_v1.joblib`) es el de 5,000 reales + 3,000 sintéticos con formantes variables. Caminos identificados para retomar: más ejemplos sintéticos, correlacionar la severidad de jitter/shimmer/entonación con el género (no solo con el individuo), pesar más los combos adversariales, pitch-normalizar antes de medir formantes (idea original, pausada), o considerar una arquitectura híbrida que le garantice un piso de peso a las features de resonancia en vez de dejarlo 100% aprendido.
